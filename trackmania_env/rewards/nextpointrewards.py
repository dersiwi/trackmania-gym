...
class NextPointRewards(RewradCalculator):
    ...
    def calculate_reward(self, observations, race_finished, other_terminations):
        ...
        reward = accum_dist_reward + race_not_finished_reward + race_finished + other_term_reward
        if self.step % self.logging_freq == 0:
            self.logger.info(f"Calculated_Rewards;{reward}:accumulated_distance;{accum_dist_reward};{i};{d}:race_not_finished;{race_not_finished_reward}:race_finished;{race_finished}:other_terminations;{other_term_reward}")
        self.step += 1
        return reward
