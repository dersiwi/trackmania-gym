...
sys.path.append(os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')), "bro")) 
print(os.path)
...
from bro.jaxrl.bro.bro_learner import BRO
from bro.jaxrl.replay_buffer import ParallelReplayBuffer
from bro.jaxrl.utils import mute_warning, log_to_wandb_if_time_to, evaluate_if_time_to, make_env
...
def main(cfg : TrainConfig, run_id : Optional[str] = None):
    ...
    assert cfg.vectorized.vectorize, "Needs to be vectoirzed"
    env = eval_env = SB3Vectorized(n_envs = cfg.vectorized.n_envs, 
                            tracks=cfg.vectorized.tracks, 
                            cfg=cfg, obs_as_dict=True, step_parallel=True, 
                            lock = Lock())
    # BRO uses evaluation environment different from actual environment; but takes too much resources.
    """wandb.init(
        config=FLAGS,
        entity='naumix',
        project='BRO',
